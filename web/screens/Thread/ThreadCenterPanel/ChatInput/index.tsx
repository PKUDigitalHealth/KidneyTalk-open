...
import { TextArea, Button, useClickOutside, Badge, Switch } from '@janhq/joi'
...
const hasId = (assistant: any): assistant is { id: string } => {
  return assistant && 'id' in assistant;
}
...
const ChatInput = () => {
  ...
  const [ifEnableRAG, setIfEnableRAG] = useState(() => {
    const saved = localStorage.getItem('ifEnableRAG')
  ...
  useEffect(() => {
    localStorage.setItem('ifEnableRAG', JSON.stringify(ifEnableRAG))
  }, [ifEnableRAG])
      ...
      <div className="relative flex w-full flex-col">
          ...
          >
            <ul>
              ...
              <Tooltip
                ...
                content={
                    <span>
                      We support PDF, TXT, and Markdown files.
                    </span>
                }
              />
              <Tooltip
                ...
                trigger={
                  <li
                    ...
                    onClick={() => {
                      // Handle the upload logic for conversation recording files
                      // You can add file upload references and handling logic here
                      // DESKLLM: 增加录音转译文件上传的逻辑
                      alert('Developing...')
                    }}
                  >
                    <StopCircle size={16} />
                ...
                content={
                  <span>
                    We recommend using the open-source software Buzz to record conversations with patients. 
                    You can find Buzz at Github: /chidiwilliams/buzz
                  </span>
                }
              />
            </ul>
        ...
        >
          <div className="flex items-center gap-x-2">
            ...
            {hasId(activeAssistant) && activeAssistant.id === 'jan' && (
              <Badge
                theme={ifEnableRAG ? "info" : "secondary"}
              ...
              <Switch
                  ...
                  checked={ifEnableRAG}
                  onChange={() => {
                    setIfEnableRAG(!ifEnableRAG)
                    if (!ifEnableRAG) {
                      toaster({
                        title: 'Docs-Enhanced mode enabled',
                        description: 'This mode may take a few seconds to retrieve knowledge for each conversation',
                        type: 'success'
                      })
                    } else {
                      toaster({
                        title: 'Docs-Enhanced mode disabled',
                      })
                    }
                  }}
